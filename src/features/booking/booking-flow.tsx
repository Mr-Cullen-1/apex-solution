...
const fieldClass = "min-h-12 w-full rounded-control border border-steel bg-surface px-4 py-3 text-base text-navy outline-none transition-colors placeholder:text-slate/65 focus:border-copper";
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem] xl:gap-16">
      <div>
        ...
        <div className="sticky bottom-0 z-20 -mx-page mt-12 flex items-center justify-between gap-3 border-t border-steel bg-page-bg/95 px-page py-4 backdrop-blur sm:static sm:mx-0 sm:bg-transparent sm:px-0 sm:py-0">
          {step > 0 ? <button type="button" className="min-h-12 rounded-control border border-steel px-6 text-sm font-semibold text-navy hover:border-navy" onClick={() => goTo(step - 1)}>Back</button> : <span />}
          {step < bookingSteps.length - 1 ? <button type="button" className="group inline-flex min-h-12 items-center gap-3 rounded-control bg-navy px-6 text-sm font-semibold text-white hover:bg-copper" onClick={continueFlow}>Continue <ArrowRightIcon className="size-4 transition-transform group-hover:translate-x-1" /></button> : <button type="button" disabled={submitting} className="min-h-12 rounded-control bg-copper px-6 text-sm font-semibold text-white hover:bg-brand-dark disabled:cursor-wait disabled:opacity-60" onClick={submit}>{submitting ? "Preparing…" : "Prepare request"}</button>}
        </div>
      </div>
...
function Progress({ step }: { step: number }) {
  return <nav aria-label="Booking progress"><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate">Request progress</p><ol className="mt-4 grid grid-cols-6 gap-1.5">{bookingSteps.map((label, index) => <li key={label} aria-current={index === step ? "step" : undefined}><span className={`block h-1.5 rounded-control ${index <= step ? "bg-copper" : "bg-steel"}`} /><span className={`mt-3 hidden text-xs font-semibold lg:block ${index === step ? "text-navy" : "text-slate"}`}>{index + 1}. {label}</span><span className="sr-only">{index + 1}. {label}{index === step ? ", current step" : ""}</span></li>)}</ol></nav>;
}
...
function Confirmation({ message, draft }: { message: string; draft: BookingDraft }) {
  ...
  return <section className="rounded-hero border-t-4 border-copper bg-brand-dark px-7 py-12 text-white sm:px-12 sm:py-16" aria-labelledby="request-ready-heading"><p className="eyebrow text-white/75">Request prepared</p><h2 ref={confirmationHeadingRef} id="request-ready-heading" tabIndex={-1} className="mt-5 max-w-3xl text-balance text-[clamp(2.25rem,5.5vw,4rem)] font-semibold leading-[1.05] tracking-[-0.03em]">Your details are ready.</h2><p className="mt-7 max-w-2xl text-lg leading-8 text-white/70">{message}</p><div className="mt-10 rounded-panel border border-white/15 bg-white/[0.04] p-7"><p className="text-xs font-bold uppercase tracking-[0.14em] text-copper">Service request</p><p className="mt-2 text-2xl font-semibold">{service?.name ?? category?.name}</p><p className="mt-3 text-sm text-white/60">Preferred date: {draft.preferredDate}. This is not a confirmed appointment.</p></div><div className="mt-9 flex flex-col gap-3 sm:flex-row"><Link href="/" className="inline-flex min-h-12 items-center justify-center rounded-control bg-white px-6 text-sm font-semibold text-navy">Return home</Link><Link href="/services" className="inline-flex min-h-12 items-center justify-center rounded-control border border-white/35 px-6 text-sm font-semibold text-white">Explore services</Link></div></section>;
}
...
const ErrorSummary = forwardRef<HTMLDivElement, { errors: BookingErrors }>(function ErrorSummary({ errors }, ref) {
  return <div ref={ref} tabIndex={-1} role="alert" className="mt-8 rounded-panel border-l-4 border-copper bg-surface p-5"><p className="font-semibold text-navy">Please review this step.</p><ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate">{Object.values(errors).filter(Boolean).map((error) => <li key={error}>{error}</li>)}</ul></div>;
});
